...
$('#dataset').change(function() {
  var tag = ''
  if ($(this).val() == 'OCT'){
      var source = 'cropped-oct106';
      var target = '13361669136864_';
      var d = 'oct';
  }
  if ($(this).val() == 'MRI-CT'){
    var source = 'pre_img0012_tcia_MRslice_132';
  ...
  if ($(this).val() == 'cardiac'){
      var source = 'patient025sa_ES_cropslice_3';
      var target = 'cardiac_translated';
      var d = 'cardiac';
  }
  init_img_vel(source, target,d,tag);
});
...
function init_img_vel(s, t, d, tag){
  ...
  img1.src = './imgs/{0}/trans/{1}.png'.formatUnicorn(d, t);
  url = git_raw_url + '/json_files/json_{0}/{1}_{2}.json'.formatUnicorn(d, tag + s, t);
  get_vel(url);
}
